...
def get_ray_directions(h, w, intrinsics, norm=False, device=None):
    ...
    x = torch.linspace(0.5, w - 0.5, w, device=device) / w
    y = torch.linspace(0.5, h - 0.5, h, device=device) / h
    # (*, h, w, 2)
...
class MeshRenderer(nn.Module):
    ...
    def bake_multiview(self, mesh, images, alphas, poses, intrinsics, map_size=1024, cos_weight_pow=4.0):
        """
        Args:
            mesh:
            images: [batch, h, w, 3]
        ...
        r_mat_c2w = torch.cat([poses[..., :3, :1], -poses[..., :3, 1:3]], dim=-1)  # opencv to opengl conversion

        proj = torch.zeros_like(poses, device=poses.device)
        proj[:, 0, 0] = 2 * intrinsics[0]
        proj[:, 1, 1] = -2 * intrinsics[1]
        proj[:, 2, 2] = -(self.far + self.near) / (self.far - self.near)
        proj[:, 2, 3] = -(2 * self.far * self.near) / (self.far - self.near)
        proj[:, 3, 2] = -1

        # (num_images, num_vertices, 3)
        v_cam = (mesh.v.detach() - poses[:, :3, 3].unsqueeze(-2)) @ r_mat_c2w
        # (num_images, num_vertices, 4)
        v_clip = F.pad(v_cam, pad=(0, 1), mode='constant', value=1.0) @ proj.transpose(-1, -2)

        v_cam = v_cam.to(torch.float32)
        v_clip = v_clip.to(torch.float32)
        ...
        depth = 1 / dr.interpolate(-v_cam[..., 2:3].contiguous(), rast, mesh.f)[0].reshape(n, h, w)
        depth.masked_fill_(~fg, 0)
        ...
        return torch.cat([new_albedo_map.clamp(min=0, max=1), torch.ones_like(new_albedo_map[..., :1])], dim=-1)
